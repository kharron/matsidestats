...
var app = {
	...
	initialize: function() {
		...
		//Load default option
		xhReq.open("GET", "/static/sitepages/options/option1.html", false);
		xhReq.send(null);
...
function menu(option){
	...
	if (option==1){
		setTitle('Matside Stats');
		xhReq.open("GET", "/", false);
	} else if (option==2){
		setTitle('Register or Login');
		xhReq.open("GET", "/register/", false);
	} else if (option==3){
		setTitle('Track Match');
		xhReq.open("GET", "/teamview/", false);
	} else {
		xhReq.open("GET", "/static/sitepages/options/option"+option+".html", false);
	}
	xhReq.send(null);
	document.getElementById("sectionContent").innerHTML=xhReq.responseText;
...
var mapObject = {
	...
	init : function(){
		...
		var markers = [];
		var latlng = new google.maps.LatLng(33.2117, -117.3258);
		var myOptions = {
			zoom: 16,
			center: latlng,

  ...
  $ionicPlatform.ready(function() {
		...
		db.transaction(function(tx) {
			tx.executeSql("select * from teams;", [], function(tx, res){
				...
				if (res.rows.length){
					window.localStorage['team'] = JSON.stringify(teams[0]);
					//console.log(window.localStorage['team']);
					$state.go('roster.teamview', {}, {reload: true});
				}
				console.log(teams);
...
.controller('rosterCtrl', function($ionicPlatform, $scope, $state, WrestlersDb, Roster, $ionicPlatform, $ionicModal, $rootScope, $ionicPopup) {
  $ionicPlatform.ready(function() {
		...
		$scope.opponent = {name: ''};
		$scope.addOpponent = function(wid, indx){
		var myPopup = $ionicPopup.show({
			...
			myPopup.then(function(data) {
				oppo_name = data[0];
				indx = data[1];
				WrestlersDb.setOpponent(oppo_name, wid, indx).then(function(wrestler){
					$scope.wrestlers[indx] = wrestler;
...
.controller('viewmatchesCtrl', function($ionicPlatform, $scope, $stateParams, MatchesDb, ViewMatches){
  $ionicPlatform.ready(function() {
